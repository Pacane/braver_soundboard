...
let createStream path =
    Process.Start(
        ProcessStartInfo(
            ...
            Arguments = $"-hide_banner -loglevel panic -c:a libvorbis -i \"{path}\" -ac 2 -f s16le -ar 44000 -filter:a \"volume=30dB\" pipe:1",
...
let sendAsync (client: IAudioClient, path) =
    task {
        ...
        use discord = client.CreatePCMStream AudioApplication.Music
